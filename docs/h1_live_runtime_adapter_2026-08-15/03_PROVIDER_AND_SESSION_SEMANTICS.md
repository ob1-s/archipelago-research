...
The request input is restricted to plain declared message items: every input item must be an object whose keys are exactly `{role, content}`, whose role is one of `user`, `system`, `developer`, `assistant`, and whose content is a string. This allowlist is enforced by the `ProviderRequest` model, so a conversation-shaped or tool-shaped input cannot earn a clean L0 (adversarial-review finding B5, closed). Request/response identity is generic, not OpenAI-specific: the evidence contract always records the gateway-owned wire attempt identity, the signed request/output hashes, the signed gateway receipt, and the nonempty response-body ID, while a provider-issued request identifier (`GatewayReceipt.provider_request_id` / `ProviderResponse.request_id`) is optional and preserved verbatim when the provider emits one (findings B2/B22, disposition below). The OpenAI adapter itself still fails closed when its response lacks a server `X-request-id`; that is a transport check for the OpenAI configuration, not a generic contract requirement.
