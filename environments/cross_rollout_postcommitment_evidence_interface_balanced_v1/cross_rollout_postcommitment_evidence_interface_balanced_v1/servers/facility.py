"""Minimal label-neutral facility for the native two-turn lifecycle.

This module intentionally does not postpone annotations.  The exposed policy
argument is a plain ``str``: K/M validation is runtime state logic, not MCP
schema information.
"""
...
class BalancedToolsetConfig(vf.ToolsetConfig):
    """No model-visible configuration knobs are needed."""


class BalancedToolset(vf.Toolset[BalancedToolsetConfig, BalancedState]):
    """Select and release one policy per resource."""
    ...
    def _set_evidence_summary(
        self, policy: Policy, acquired: bool, verified: bool
    ) -> None:
    ...
    def _fix_neutral_fail_control(self) -> None:
        state = self.state
        state.assignment_status = "fixed_neutral_nonprimary"
        state.assignment_randomized = False
        state.exposure_condition = "neutral"
        state.phase2_presentation_order = state.secondary_phase2_order
        state.exposure_text = NEUTRAL_PREDECESSOR_SENTENCE
        event = record_event(
            state,
            "assignment",
            "after_r1_success_verification_fail",
    ...
    def _assign_primary(self) -> None:
        state = self.state
        if state.phase1_policy is None:
            raise RuntimeError("primary assignment requires a Phase-1 policy")
        if state.eligibility != "primary_eligible":
            raise RuntimeError("primary assignment requires fixed eligibility")
        ...
        state.random_draws["phase2_assignment_block"] = DrawRecord(
            ...
            key=allocated.order_key,
            value=float(allocated.slot),
            threshold=None,
    ...
    async def select_policy(self, policy: str) -> str:
        ...
        state.policy_selection_attempts.append({"phase": "phase2", "policy": policy})
        if state.first_phase2_policy_call is None:
            state.first_phase2_policy_call = policy
            if policy not in ("K", "M"):
                state.phase2_selection_invalid = True
                result = (
                    "The first R2 policy selection was invalid. R2 cannot be "
                    "selected after an invalid first choice."
                )
    ...
    async def release_resource(self) -> str:
        ...
        if state.phase == "phase1":
            ...
            self._set_evidence_summary(selected, acquired, verified)
            ...
            if not verified:
                ...
                self._fix_neutral_fail_control()
        ...
        state.phase = "complete"
        if state.primary_eligible and state.evidence_supported_policy is not None:
