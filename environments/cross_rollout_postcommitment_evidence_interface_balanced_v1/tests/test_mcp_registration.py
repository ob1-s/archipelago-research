...
MODULE = (
    "cross_rollout_postcommitment_evidence_interface_balanced_v1.servers.facility"
)
...
def test_real_subprocess_mcp_schema_is_string_only_and_label_free(tmp_path: Path) -> None:
    ...
    try:
        ...
        release = next(
            tool for name, tool in tools.items() if name.endswith("release_resource")
        )
        ...
        assert policy_schema == {"title": "Policy", "type": "string"}
        assert "enum" not in json.dumps(schema)
        assert "oneOf" not in json.dumps(schema)
        assert not any(label in json.dumps(schema) for label in ("K", "M"))
