...
MODULE = "cross_rollout_postcommitment_provenance_dose_response_v1.servers.facility"
...
def test_real_subprocess_mcp_schema_is_string_only_and_label_free(tmp_path: Path) -> None:
    ...
    env.update(
        {
            ...
            "VF_CONFIG": json.dumps({"colocated": False, "runtime": {"type": "subprocess"}}),
            "PYTHONUNBUFFERED": "1",
        }
    )
    ...
    try:
        ...
        if port is None:
            ...
            raise AssertionError(f"MCP server did not bind; returncode={process.returncode}\n{stderr}")
...
async def _list_tools(url: str):
    async with streamable_http_client(url) as (read_stream, write_stream, _), ClientSession(
        read_stream,
        write_stream,
