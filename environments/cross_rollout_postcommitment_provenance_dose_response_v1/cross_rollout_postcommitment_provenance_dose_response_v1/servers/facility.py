...
class DoseResponseToolsetConfig(vf.ToolsetConfig):
    """No model-visible configuration knobs are needed."""


class DoseResponseToolset(vf.Toolset[DoseResponseToolsetConfig, DoseResponseState]):
    ...
    def _assign_primary(self) -> None:
        state = self.state
        if state.phase1_policy is None or state.eligibility != "primary_eligible":
        ...
        state.assignment_key = allocated.source_pair_key
        state.assignment_order_key = allocated.assignment_order_key
        state.source_pair_key = allocated.source_pair_key
        state.pair_id = allocated.pair_id
        state.source_condition = allocated.source_condition
        state.advisory_reliability = allocated.advisory_reliability
        state.phase2_presentation_order = allocated.phase2_order
        state.source_text = source_sentence_for(allocated.source_condition)
        event = record_event(
            state,
            "assignment",
            "after_primary_evidence",
            f"{allocated.source_condition}@{allocated.advisory_reliability:.2f}",
    ...
    async def release_resource(self) -> str:
        ...
        if state.phase == "phase1":
            ...
            verified = self._draw_outcome(
                "r1_verification",
                MATCH_VERIFICATION_PROBABILITY if matched else MISMATCH_VERIFICATION_PROBABILITY,
                matched,
            )
