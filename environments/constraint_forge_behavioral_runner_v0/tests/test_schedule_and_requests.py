...
def test_frozen_plan_contains_14_ordinary_4_fault_and_6_late_probe_slots() -> None:
    ...
    assert [job.rack_condition for job in probes].count("both_visible") == 2
    assert [job.rack_condition for job in probes].count("both_hidden") == 2
    assert [job.rack_condition for job in probes].count("x_hidden") == 1
    assert [job.rack_condition for job in probes].count("y_hidden") == 1
