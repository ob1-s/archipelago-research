"""Native two-turn source-provenance / evidence-integration taskset."""
...
from .servers.facility import ProvenanceToolset, ProvenanceToolsetConfig
from .state import DrawRecord, ProvenanceState, record_event


def activate_r2(state: ProvenanceState) -> None:
    ...
    if state.source_condition is None or state.phase2_presentation_order is None:
        raise RuntimeError("R2 activation requires fixed source and order")
    ...
    assigned = (state.source_condition, state.phase2_presentation_order)
    ...
    if (state.source_condition, state.phase2_presentation_order) != assigned:
        raise RuntimeError("R2 activation changed source or order")


class ProvenanceTaskData(vf.TaskData):
    attempt_index: int
    random_seed: str
    assignment_seed: str
...
class ProvenanceTaskConfig(vf.TaskConfig):
    tools: ProvenanceToolsetConfig = ProvenanceToolsetConfig()


class ProvenanceTask(vf.Task[ProvenanceTaskData, ProvenanceState, ProvenanceTaskConfig]):
    @classmethod
    def toolsets(cls, config: ProvenanceTaskConfig) -> list[vf.Toolset]:
        return [ProvenanceToolset(config.tools)]
    ...
    async def setup(self, trace: vf.Trace, runtime: vf.Runtime) -> None:
        ...
        trace.info["provenance_assay"] = {
            ...
            "source_visible_before_turn2": False,
        ...
        if current >= TARGET_PRIMARY_ELIGIBLE:
            ...
            trace.info["provenance_assay"].update(
                {
...
class ProvenanceConfig(vf.TasksetConfig):
    random_seed: str = "cross-rollout-postcommitment-provenance-v1-random-2026-08-14"
    assignment_seed: str = "cross-rollout-postcommitment-provenance-v1-assignment-2026-08-14"
    assignment_state_path: str = "/tmp/cross-rollout-postcommitment-provenance-v1-assignments-2026-08-14.json"
    schedule_seed: str = SCHEDULE_SEED
    task: ProvenanceTaskConfig = ProvenanceTaskConfig()


class CrossRolloutPostcommitmentProvenanceV1Taskset(
    vf.Taskset[ProvenanceTask, ProvenanceConfig]
):
    def load(self) -> list[ProvenanceTask]:
        schedule = build_schedule(self.config.schedule_seed)
        if len(schedule) != MAX_ATTEMPTS:
            raise RuntimeError("frozen schedule is not exactly 520 attempts")
        return [
            ProvenanceTask(
                ProvenanceTaskData(
                    idx=spec.attempt_index,
                    name=f"provenance-{spec.attempt_index:03d}-{spec.phase1_order}",
...
class ProvenanceEnvConfig(vf.EnvConfig):
...
class CrossRolloutPostcommitmentProvenanceV1Env(vf.Env[ProvenanceEnvConfig]):
    async def run(self, task: ProvenanceTask, agents: vf.Agents) -> None:
        async with agents.agent.interaction(task) as interaction:
            ...
            if state.source_condition is None or state.phase2_presentation_order is None:
                raise RuntimeError("R2 activation lacks source/order")
            state.turn2_sent_count += 1
            state.turn2_message = phase2_prompt(
                state.phase2_presentation_order, state.source_condition
            )
            if state.source_condition != "NoAdvisory":
                state.advisory_delivered = True
                state.source_text = (
                    PREDECESSOR_SOURCE_SENTENCE
                    if state.source_condition == "PredecessorSource"
            ...
            if state.advisory_delivered and turn2_event.index != state.exposure_event_index + 1:
