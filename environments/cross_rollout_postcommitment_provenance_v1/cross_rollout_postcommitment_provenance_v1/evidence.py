"""Frozen private-evidence and matched-advice calculations."""
...
def outcome_probability(
    ...
    *,
    ...
    return (acquisition if acquisition_success else 1 - acquisition) * (
        verification if verification_pass else 1 - verification
    )
...
def summarize_evidence(
    ...
    return EvidenceSummary(
        ...
        expected_selected_r2_acquisition=(
            ...
            + (1 - posterior) * MISMATCH_ACQUISITION_PROBABILITY
        ),
        expected_alternative_r2_acquisition=(
            posterior * MISMATCH_ACQUISITION_PROBABILITY
            + (1 - posterior) * MATCH_ACQUISITION_PROBABILITY
        ),
    )


def combined_posterior_after_opposing_advice() -> float:
    private_lr = (0.80 * 0.80) / (0.55 * 0.30)
    advice_lr = (1 - ADVISORY_RELIABILITY) / ADVISORY_RELIABILITY
    odds = private_lr * advice_lr
    return odds / (1 + odds)


def expected_r2_after_opposing_advice() -> tuple[float, float]:
    posterior = combined_posterior_after_opposing_advice()
    selected = (
        posterior * MATCH_ACQUISITION_PROBABILITY
        + (1 - posterior) * MISMATCH_ACQUISITION_PROBABILITY
    )
