"""Minimal facility tools for the source-provenance assay."""
...
class ProvenanceToolsetConfig(vf.ToolsetConfig):
    """No model-visible configuration knobs are needed."""


class ProvenanceToolset(vf.Toolset[ProvenanceToolsetConfig, ProvenanceState]):
    ...
    def _assign_primary(self) -> None:
        ...
        if state.eligibility != "primary_eligible":
            raise RuntimeError("source assignment requires fixed eligibility")
        ...
        state.assignment_key = allocation.assignment_key
        state.source_condition = allocation.condition
