...
def draw_digest(seed: str, rollout_id: str, namespace: str) -> bytes:
    ...
    if namespace not in RANDOM_NAMESPACES:
        raise ValueError(f"unknown frozen randomness namespace: {namespace}")
...
def assigned_condition(seed: str, rollout_id: str) -> tuple[Literal["neutral", "culture-A"], float, str]:
    """Return an independent 1:1 deferred assignment draw."""

    digest = draw_digest(seed, rollout_id, "treatment_assignment")
    value = int.from_bytes(digest[:8], "big") / float(2**64)
    condition: Literal["neutral", "culture-A"] = (
        "neutral" if digest[0] % 2 == 0 else "culture-A"
    )
    key = f"sha256({seed}:{rollout_id}:treatment_assignment)[0]%2"
