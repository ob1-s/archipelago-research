...
from constraint_forge_behavioral_runner_r2._r2_world.actions import FinishAction, SetAction
from constraint_forge_behavioral_runner_r2.failures import (
    BehavioralCallFailure,
    FailureClass,
    FailureEvidence,
)
from constraint_forge_behavioral_runner_r2.harness import (
    CALL_TIMEOUT_SECONDS,
    TEXT_PROGRAM_SOURCE,
    ConstraintForgeTextHarness,
    text_program_source,
)
from constraint_forge_behavioral_runner_r2.requests import memory_request
from constraint_forge_behavioral_runner_r2.runner import (
    SequenceResult,
    run_behavioral_sequence,
    stamp_sequence_traces,
)
from constraint_forge_behavioral_runner_r2.taskset import (
...
from constraint_forge_behavioral_runner_r2._r2_world.rack import full_rack_view
from constraint_forge_behavioral_runner_r2._r2_world.world import run_job
...
class _FakeInteraction:
    ...
    async def turn(self, prompt: str):
        ...
        if payload["phase"] == "round":
            ...
            registers = payload["observation"]["registers"]
            mask = tuple(tuple(pair) for pair in payload["observation"]["private_pairs"])
            target = self.actor.target_by_mask[mask]
            if (
                role == "Y"
                and registers["X"][0]["symbol"] is not None
                and registers["Y"][0]["symbol"] is None
            ):
                raw = {
                    "action": "write",
                    "register": 0,
                    "symbol": registers["X"][0]["symbol"],
                }
            elif role == "X" and all(
                entry["symbol"] is None for entry in registers["X"]
            ):
                marker = "void symbol for register 0 is "
                if self.actor.force_void_collision:
                    raw = {"action": "write", "register": 0, "symbol": 0}
                elif marker in payload["instructions"]:
                    raw = {"action": "write", "register": 0, "symbol": (int(payload["instructions"].split(marker)[1][0]) + 1) % 4}
                else:
                    raw = {"action": "write", "register": 0, "symbol": 1}
            else:
...
class _FakeActor:
    def __init__(self, target_by_mask, *, delay: float = 0.0, rotate_memory: bool = False, fail_class=None, force_void_collision: bool = False):
        self.target_by_mask = target_by_mask
        self.delay = delay
        self.rotate_memory = rotate_memory
        self.fail_class = fail_class
        self.force_void_collision = force_void_collision
        self.total_calls = 0
...
def test_read_only_sequence_job_has_no_memory_calls() -> None:
    ...
    result, x, y = asyncio.run(run())
    assert result.handoff.run_valid
    assert len(x.contexts[0]) == len(y.contexts[0]) == 8
