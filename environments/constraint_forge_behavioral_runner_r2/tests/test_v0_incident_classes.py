...
environments/constraint_forge_behavioral_runner_r2/docs/phaseA_forensics.md
...
def test_probe_schedule_pairs_contrast_film_conditions():
    ...
    from constraint_forge_behavioral_runner_r2.schedule import build_run_plan
    ...
    for sequence_index in range(4):
        ...
        for job in plan.jobs:
            if job.intervention is not None:
                from constraint_forge_behavioral_runner_r2._r2_world.interventions import (
                    InterventionKind,
                )
...
def test_preflight_flags_dead_bridge(tmp_path: Path):
    ...
    proc = subprocess.run(
        [
            sys.executable, "-m",
            "constraint_forge_behavioral_runner_r2.cohort_launcher",
