...
from ._config import retain_start_cap
...
class RetainAction(ActionModel):
    ...
    start_round: Annotated[StrictInt, Field(ge=1)]

    @field_validator("start_round")
    @classmethod
    def _window_fits_within_job(cls, value: int) -> int:
        cap = retain_start_cap()
        if value > cap:
            raise ValueError(f"start_round exceeds configured retention domain 1-{cap}")
        return value
