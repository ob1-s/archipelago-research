...
from constraint_forge_behavioral_runner_r2._r2_world.models import StrictModel, Seed
...
class FormationJobReceipt(StrictModel):
    ...
    job_seed: Seed
    success: StrictBool
    world_success: StrictBool = False
    void_symbol: StrictInt | None = Field(default=None, ge=0, le=3)
    x_register0_final: StrictInt | None = Field(default=None, ge=0, le=3)
    y_register0_final: StrictInt | None = Field(default=None, ge=0, le=3)
    ...
    rack_wipe: RackWipeRecord | None = None


class RackWipeRecord(StrictModel):
    """Evidence of a V1 film-wipe manipulation on one probe job."""

    schema_version: Literal["constraint-forge/runner-rack-wipe/v1"] = (
        "constraint-forge/runner-rack-wipe/v1"
    )
    job_index: StrictInt = Field(ge=0)
    prior_films_x: StrictInt = Field(ge=0)
    prior_films_y: StrictInt = Field(ge=0)
    prior_content_hash_x: StrictStr | None = None
    prior_content_hash_y: StrictStr | None = None
    wiped_content_hash_x: StrictStr
    wiped_content_hash_y: StrictStr
