...
def phase1_prompt(order: PolicyOrder) -> str:
    """Return the cleaned Phase-1 prompt with only the explicit K/M ordering."""
...
def _q_percent(q: float) -> str:
    if q not in ADVISORY_RELIABILITY_LEVELS:
        raise ValueError("advisory reliability is not in the frozen nine-level grid")
    return f"{q:.2%}"


def source_sentence_for(source: SourceCondition) -> str:
    if source == "PredecessorSource":
        return PREDECESSOR_SOURCE_SENTENCE
    if source == "AutomatedSource":
        return AUTOMATED_SOURCE_SENTENCE
...
def phase2_prompt(
    ...
    """Return the frozen Turn-2 message for a q/source/order cell."""
