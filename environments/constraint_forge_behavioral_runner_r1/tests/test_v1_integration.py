...
from constraint_forge_behavioral_runner_r1.harness import ConstraintForgeTextHarnessConfig
...
class _DeterministicClient(Client):
    ...
    async def get_response(
        ...
        if request["phase"] == "round":
            ...
            elif request["role"] == "X" and all(
                entry["symbol"] is None for entry in registers["X"]
            ):
                marker = "void symbol for register 0 is "
                instructions = request["instructions"]
                symbol = (
                    (int(instructions.split(marker)[1][0]) + 1) % 4
                    if marker in instructions
                    else 1
                )
                answer = {"action": "write", "register": 0, "symbol": symbol}
            else:
                answer = next(
                    (
